...
void Model::render(bool texturebool)
{
...
bool Model::loadModel(char* filename)
{
	...
	for (int i = 0; i < faces.size(); i += 3) {
		vIndex = faces[i] - 1;
		vertex.push_back(verts[vIndex].x);
		vertex.push_back(verts[vIndex].y);
		vertex.push_back(verts[vIndex].z);

		vIndex = faces[i + 1] - 1;

		texCoords.push_back(texCs[vIndex].x);
		texCoords.push_back(texCs[vIndex].y);

		vIndex = faces[i + 2] - 1;

		normals.push_back(norms[vIndex].x);
		normals.push_back(norms[vIndex].y);
		normals.push_back(norms[vIndex].z);
